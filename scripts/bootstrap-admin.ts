...
if (!connectionString) {
  throw new Error("No database connection string was found for admin bootstrap.");
}
...
async function main() {
  ...
  const email = readArg("--email") ?? DEFAULT_ADMIN_EMAIL;
  ...
  console.log(`Admin ready: ${user.username} / ${password}`);
}
